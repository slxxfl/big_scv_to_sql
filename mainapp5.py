...
class MY_C:
    ...
    def __init__(self):
        # 配置基本参数
        self.DATABASE_NAME = 'mydb_01'  # 数据库名
        self.TABLE_NAME = 'mytable_01'  # 数据表名
        self.DB_ADMIN = 'root'  # 数据库管理员账号
        self.DB_PW = '123123qwe'  # 数据库管理员密码
        self.SHEET_NAME = 'sheet1'  # 导入表的名称
        ...
        self.NAME_COL = 4  # NAME所在的列
        self.PID_COL = 5  # pid所在的列
        self.CSV_PATH = os.path.abspath('./') + '/res/' + self.CSV_NAME
        self.ADDorNew = False  # Flase 表示全新建表，True 表示增量加纪录
    ...
    def insert_data(self):
        ...
        try:
            ...
            first_data = [self.SHEET_NAME, self.WRPW, self.RPW,
                          firstrow[self.NAME_COL - 1], firstrow[self.PID_COL - 1]]
            # 这一条等价于first.extend(firstrow) 特别的，这个函数是没有返回值的，只是在原列表的基础上增加参数列表。
            first_data += firstrow
